...
public enum CleanActionType: Equatable, Sendable {
    ...
    case paths([URL], deleteEntireFolder: Bool)
...
public class DeepCleanViewModel: ObservableObject {
    ...
    private func setupCategories() {
        ...
        var initialCategories: [JunkCategory] = [
            ...
            JunkCategory(name: "Trash", description: "Files you've moved to the Trash.", isSelected: true, needsAdmin: false, actionType: .paths([
                homeDir.appendingPathComponent(".Trash"),
                homeDir.appendingPathComponent("Library/Mobile Documents/.Trash")
    ...
    public func scan() async {
        ...
        for i in 0..<updatedCategories.count {
            ...
            case .paths(let urls, _):
                var total: Int64 = 0
                for url in urls {
                    total += await calculateSize(of: url)
                }
                updatedCategories[i].size = total
    ...
    public func clean() async {
        ...
        let resultError = await Task.detached {
            ...
            for category in selectedCategories {
                ...
                case .paths(let paths, let deleteEntireFolder):
                    for path in paths {
                        if category.needsAdmin {
                            adminPathsToDelete.append(path.path)
                        } else {
